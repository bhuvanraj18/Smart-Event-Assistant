...
const BUILDERS={flower:buildFlowerVase,chandelier:buildChandelier,balloons:buildBalloonArch,table:buildDecoratedTable,stage:buildStageBackdrop};
    ...
    <>
            ...
            <div className="card" style={{ padding: '1.5rem' }}>
              {!uploadedImage && !externalImage && (
                <div style={{ marginBottom: '1rem' }}>
                  <p style={{color:'var(--text-muted)',fontSize:'0.75rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:'0.75rem'}}>Choose a 3D Decoration</p>
                  <div className="ar-model-selector">
                    {MODELS.map(mod=>(
                      <button key={mod.id} onClick={()=>{setSelectedModel(mod.id);setUploadedImage(null);onClearExternal?.();}} className={`btn ${selectedModel===mod.id?'btn-primary':'btn-outline'}`} style={{padding:'0.5rem 1rem',fontSize:'0.8rem',textTransform:'none',letterSpacing:'0'}}>
              ...
              <div className="ar-info-box">
                <p>
                  💡 <strong style={{color:'var(--text-dark)'}}>How it works:</strong><br/>
                  • <strong>Mobile AR:</strong> Uses your phone's AR to place & lock the decoration in your real room<br/>
                  • <strong>Desktop Camera:</strong> Opens webcam with motion tracking — the decoration stays fixed as you move the camera
                </p>
              </div>
            </div>
          </div>
      ...
      {cameraActive && (
        <div className="ar-fullscreen">
          <video ref={videoRef} autoPlay playsInline muted style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',zIndex:0}}/>
