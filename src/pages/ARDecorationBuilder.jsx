...
import { deleteScene, exportSceneAsJSON, importSceneFromJSON, listSavedScenes, loadScene, saveScene } from '../utils/sceneManager';
...
const ARDecorationBuilder = () => {
  ...
  // ─── THREE.JS SETUP ───
  useEffect(() => {
    setSavedScenes(listSavedScenes());
  }, []);
...
- Return ONLY the raw JSON array, no markdown, no explanation.`;
  ...
  const handleSaveScene = () => {
    if (!sceneName.trim() || objects.length === 0) {
      alert('Please enter a name and generate a scene first');
      return;
    }
    const result = saveScene(sceneName, objects, imageB64, { style: styleSelect, count: countSelect });
    if (result.success) {
      setSavedScenes(listSavedScenes());
      setSceneName('');
      setShowSaveDialog(false);
      setStatus({ type: 'success', msg: `✓ Scene saved: "${sceneName}"`, spin: false });
    } else {
      setStatus({ type: 'error', msg: `✗ Failed to save: ${result.error}`, spin: false });
    }
  };
  ...
  const handleLoadScene = (sceneId) => {
    ...
    if (scene) {
      ...
      buildScene(scene.objects);
      if (scene.imageB64) setImageB64(scene.imageB64);
      setStatus({ type: 'success', msg: `✓ Loaded: "${scene.name}"`, spin: false });
    } else {
      setStatus({ type: 'error', msg: '✗ Failed to load scene', spin: false });
    }
  };

  const handleDeleteScene = (sceneId) => {
    if (window.confirm('Delete this saved scene?')) {
      deleteScene(sceneId);
      setSavedScenes(listSavedScenes());
      setStatus({ type: 'success', msg: '✓ Scene deleted', spin: false });
    }
  };

  const handleExportScene = (scene) => {
    exportSceneAsJSON(scene);
    setStatus({ type: 'success', msg: `✓ Scene exported: "${scene.name}"`, spin: false });
  };

  const handleImportScene = async (file) => {
    try {
      const scene = await importSceneFromJSON(file);
      setObjects(scene.objects);
      buildScene(scene.objects);
      if (scene.imageB64) setImageB64(scene.imageB64);
      setStatus({ type: 'success', msg: `✓ Scene imported: "${scene.name}"`, spin: false });
    } catch (err) {
      setStatus({ type: 'error', msg: `✗ Import failed: ${err.message}`, spin: false });
    }
  };
    ...
    <div className="ar-decoration-builder">
      ...
      <div className="builder-main">
        ...
        <div className="builder-sidebar">
          ...
          {objects.length > 0 && (
            <div className="builder-panel">
              <div className="builder-panel-title">Save & Load</div>
              <div className="builder-flex-row">
                <button
              ...
              {showSaveDialog && (
                <div style={{ marginTop: '0.75rem', padding: '0.75rem', background: '#13131a', borderRadius: '6px', border: '1px solid #2a2a3a' }}>
                  <input
                    type="text"
                    placeholder="Scene name..."
                    value={sceneName}
                    onChange={(e) => setSceneName(e.target.value)}
                    style={{ width: '100%', padding: '0.4rem', marginBottom: '0.5rem', background: '#1c1c27', border: '1px solid #2a2a3a', borderRadius: '4px', color: '#f1f0ff', fontSize: '0.75rem' }}
                  />
                  <div className="builder-flex-row">
                    <button className="builder-btn builder-btn-primary" onClick={handleSaveScene} style={{ fontSize: '0.7rem', padding: '0.4rem 0.6rem' }}>✓ Save</button>
                    <button className="builder-ctrl-btn" onClick={() => setShowSaveDialog(false)} style={{ fontSize: '0.7rem', padding: '0.4rem 0.6rem' }}>✕ Cancel</button>
                  </div>
                </div>
              )}
              {savedScenes.length > 0 && (
                <div style={{ marginTop: '0.75rem' }}>
                  <label className="builder-label-small">Saved Scenes</label>
                  <div style={{ maxHeight: '150px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    {savedScenes.map(scene => (
                      <div key={scene.id} style={{ padding: '0.5rem', background: '#13131a', borderRadius: '4px', border: '1px solid #2a2a3a', fontSize: '0.7rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ color: '#f1f0ff', flex: 1 }}>{scene.name}</span>
                        <div style={{ display: 'flex', gap: '4px' }}>
                          <button onClick={() => handleLoadScene(scene.id)} style={{ padding: '2px 6px', fontSize: '0.65rem', background: '#c084fc44', border: '1px solid #c084fc', borderRadius: '3px', color: '#c084fc', cursor: 'pointer' }}>Load</button>
                          <button onClick={() => handleExportScene(scene)} style={{ padding: '2px 6px', fontSize: '0.65rem', background: '#81 8cf844', border: '1px solid #818cf8', borderRadius: '3px', color: '#818cf8', cursor: 'pointer' }}>Export</button>
                          <button onClick={() => handleDeleteScene(scene.id)} style={{ padding: '2px 6px', fontSize: '0.65rem', background: '#f8717144', border: '1px solid #f87171', borderRadius: '3px', color: '#f87171', cursor: 'pointer' }}>Del</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <label style={{ marginTop: '0.75rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.4rem 0.6rem', background: '#13131a', borderRadius: '4px', border: '1px solid #2a2a3a', fontSize: '0.7rem', color: '#f1f0ff', justifyContent: 'center' }}>
                📂 Import Scene
                <input type="file" accept=".json" onChange={(e) => e.target.files?.[0] && handleImportScene(e.target.files[0])} style={{ display: 'none' }} />
              </label>
            </div>
          )}
        </div>
        ...
        <div className="builder-viewer" id="viewerPane">
          {comparisonMode && imageB64 ? (
            <div style={{ display: 'flex', height: '100%', width: '100%' }}>
              <div style={{ flex: 1, position: 'relative', overflow: 'hidden', borderRight: '2px solid var(--border)' }}>
                <img src={`data:image/jpeg;base64,${imageB64}`} alt="Original" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                <div style={{ position: 'absolute', top: '1rem', left: '1rem', background: '#13131acc', backdropFilter: 'blur(12px)', border: '1px solid #2a2a3a', borderRadius: '8px', padding: '0.5rem 0.75rem', fontSize: '0.75rem', color: '#f1f0ff', fontWeight: 600 }}>📷 Before (Original)</div>
              </div>
              <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
                <canvas ref={canvasRef} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove} onWheel={handleWheel} style={{ display: 'block', width: '100%', height: '100%' }} />
                <div style={{ position: 'absolute', top: '1rem', right: '1rem', background: '#13131acc', backdropFilter: 'blur(12px)', border: '1px solid #2a2a3a', borderRadius: '8px', padding: '0.5rem 0.75rem', fontSize: '0.75rem', color: '#f1f0ff', fontWeight: 600 }}>🎨 After (3D)</div>
              </div>
            </div>
          ) : (
            <>
